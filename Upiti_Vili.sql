...
-- ==============================================================================
-- 👤 ČLAN TIMA: Vilibald Kovac
-- ==============================================================================

-- ==============================================================================
-- 1. POGLED (VIEW 1): KATALOG IMOVINE S TRENUTNIM CIJENAMA
--
-- POSLOVNI PROBLEM:
-- Unutar aplikacije na ekranu "Katalog imovine" potrebno je klijentima prikazati 
-- popis svih imovina, ali isključivo s njihovom zadnjom (najnovijom) 
-- cijenom. Sustav mora ignorirati stare povijesne cijene kako korisnici ne bi 
-- vidjeli zastarjele podatke pri trgovanju.
-- ==============================================================================

CREATE VIEW katalog_imovine_v1 AS
SELECT 
    i.ime, 
    i.oznaka_imovine, 
    ti.tip, 
    ti.razina_rizika, 
    ROUND(pci.cijena, 2) AS trenutna_cijena
FROM imovina AS i 
...
-- Dohvat svih podataka iz kreiranog pogleda (Katalog imovine)
SELECT * FROM katalog_imovine_v1;


-- ==============================================================================
-- 2. POGLED (VIEW 2): MJESEČNI VOLUMEN TRGOVANJA
--
-- POSLOVNI PROBLEM:
-- Uprava želi pratiti mjesečni volumen trgovanja na platformi. Pod "trgovanjem" 
-- se podrazumijevaju isključivo Kupnja (3) i Prodaja (4) imovine. Uplate i isplate 
-- novca te dividende moraju se ignorirati jer to nije aktivno trgovanje. 
-- Na kraju, uprava želi vidjeti samo one mjesece u kojima je ukupni volumen 
-- trgovanja bio veći od 5.000 EUR.
-- ==============================================================================

CREATE VIEW mjesecni_volumen_trgovanja_v1 AS
SELECT 
...
WHERE t.tip_transakcije_id = 3 OR t.tip_transakcije_id = 4
GROUP BY YEAR(t.datum), MONTH(t.datum)
HAVING SUM(t.iznos) > 5000
ORDER BY godina DESC, mjesec DESC;

-- Dohvat svih podataka iz kreiranog pogleda (Volumen trgovanja)
SELECT * FROM mjesecni_volumen_trgovanja_v1;
...
-- ==============================================================================
-- 1. POGLED (VIEW 1): MJESEČNI VOLUMEN TRGOVANJA
--
-- POSLOVNI PROBLEM:
-- Uprava želi pratiti mjesečni volumen trgovanja na platformi. Pod "trgovanjem" 
-- se podrazumijevaju isključivo Kupnja (3) i Prodaja (4) imovine. Uplate i isplate 
...
-- ==============================================================================
-- 🔎 UPIT 1: IDENTIFIKACIJA MJESECI S IZNADPROSJEČNIM PROMETOM
--
-- POSLOVNI PROBLEM:
-- Nadovezujući se na prethodni View, Uprava želi izolirati samo one mjesece 
-- u kojima je ostvaren promet veći od ukupnog prosjeka svih mjeseci, 
-- kako bi se detaljnije analizirali najuspješniji periodi na platformi.
--
-- GRADIVO: Korištenje postojećeg VIEW-a, agregacija (AVG) i ugniježđeni podupit.
-- ==============================================================================
...
SELECT 
    ...
    mjesec,
    ukupni_promet
FROM mjesecni_volumen_trgovanja_v1
WHERE ukupni_promet > (
    SELECT AVG(ukupni_promet) 
    FROM mjesecni_volumen_trgovanja_v1
)
ORDER BY ukupni_promet DESC;


-- ==============================================================================
-- 2. POGLED (VIEW 2): KATALOG IMOVINE S TRENUTNIM CIJENAMA
--
-- POSLOVNI PROBLEM:
-- Unutar aplikacije na ekranu "Katalog imovine" potrebno je klijentima prikazati 
-- popis svih imovina, ali isključivo s njihovom zadnjom (najnovijom) 
...
-- ==============================================================================
-- 🔎 UPIT 2: REAKTIVACIJA KORISNIKA (MARKETING - ANTI JOIN)
--
-- POSLOVNI PROBLEM:
-- Odjel marketinga želi pokrenuti kampanju za reaktivaciju korisnika. 
-- CILJ: Pronaći sve klijente koji su otvorili investicijski račun, ali na njemu 
-- još uvijek nisu izvršili niti jednu jedinu transakciju kupnje (tip 3) ili prodaje (tip 4).
--
-- GRADIVO: LEFT JOIN s provjerom IS NULL (Anti-Join).
-- ==============================================================================
...
SELECT DISTINCT
    ...
    ir.broj_racuna
FROM klijent AS k
JOIN investicijski_racun AS ir ON (k.id = ir.klijent_id)
LEFT JOIN transakcija AS t ON (
    ir.id = t.investicijski_racun_id 
    AND (t.tip_transakcije_id = 3 OR t.tip_transakcije_id = 4)
)
WHERE t.id IS NULL;